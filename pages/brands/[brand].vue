  ...
  <div>
    ...
    <div class="container">
      <div v-if="brands?.length === 0">
        <p class="text-3xl text-center font-bold text-gray-300 p-10">No brands found</p>
      </div>

      <div v-else class="grid grid-cols-2 lg:grid-cols-6 gap-6 justify-center items-center text-center mb-20">
        <div v-for="brand in brands" :key="brand.id" class="group relative inline-block">
          ...
          <a @mouseenter="tooltip(brand.title)" :href="brand.acf.link" target="_blank"
